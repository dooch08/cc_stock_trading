...
function BalancePage() {
  ...
  useEffect(() => {
    api.get("/customer/balance").then(res => setBalance(res.data));
  }, []);

  if (!balance) return <div className="text-center mt-20 text-lg font-semibold">로딩 중...</div>;

  return (
    <div className="min-h-screen bg-gray-100 py-10 px-4">
      <div className="max-w-4xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-lg space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold">📊 잔고 조회</h2>
          <button
        ...
        <div className="grid grid-cols-2 gap-4 text-lg font-medium">
          <div>💰 현금: <span className="text-blue-600">{balance.cash.toLocaleString()} 원</span></div>
          <div>📈 양도소득: <span className="text-green-600">{balance.capital_gain.toLocaleString()} 원</span></div>
        </div>
